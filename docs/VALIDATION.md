...
| `tabs` | The terminal's tabs agree on which is showing after adding, moving, and closing, and a selection taken from the screen reaches the clipboard: `2 open after adding three and closing one, showing 1, and selecting copied 17 byte(s)`. The fixture types the line it copies rather than waiting for a prompt, because a prompt arrives when the shell is ready and that is not a frame anyone can name - on a cold runner it is seconds away. It writes that line once at the start and again every sixty frames until the copy takes, for a tab that has no shell at all yet: a pty accepts a write before its program runs, but a tab opened a moment ago may have nothing to write to, and a fixture that talks to nothing on the first try never talks again. That second write is what fixed the macOS job, which had been failing here while every Linux shell tried - delayed, unset, and nonexistent alike - copied. A run that cannot copy says why: the check prints every line the fixture reported, because the copy line has two ways to go missing - nothing on the screen, or a platform that would not take the text - and the status names which. |
...
| `strips` | A strip holds more tabs than it can show, and scrolling is how the ones past the edge stay reachable. The gate runs `--exercise-strips`, which starts ten shells in the dock and every configured lane, and reads the frame: the dock's strip is scrolled with its leftmost tab cut mid-width, the active tab whole at the right, and the new-tab control still at the far end where a full strip cannot push it off. The lane strip is checked the same way by running the same exercise against a config naming ten agents, since the default four fit and would show nothing. The range itself is pinned by native tests: a strip whose tabs fit does not scroll, one whose tabs do not fit stops exactly at its last tab, and an offset is clamped so neither the wheel nor a selection can leave a strip showing nothing. |
