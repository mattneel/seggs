...
plain JavaScript, and the declared types are checked by `npm run typecheck` in
`extensions/` rather than by the bundler, which strips them without reading
them. Authoring a bundle by hand in JavaScript is the same language.
...
| `seggs.ui.on(event, handler)` | Subscribes to `click`, `activate`, `key`, or `hover`. Subscriptions accumulate. |
...
| `key` | `panel`, `id`, `key` | Any other key while the panel owns the keyboard, other than a key held with `Ctrl`. |
| `hover` | `panel`, `id`, `x`, `y` | The pointer moved onto a node, or from one node to another. Leaving a node for empty space is not reported. Only a change is reported, so a handler does not run for every motion. |
...
those three keys stay with the interface so an extension cannot trap them, and a
key held with `Ctrl` is taken by the editor's own shortcuts before any panel
sees it. Every other key is reported to the focused node.
...
| `src/ui/yoga.h` | The only file under `src/` that names a Yoga header; the C++ stays behind it and Zig sees the `yoga` module. |
