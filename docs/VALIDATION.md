...
| Transcript tool calls | A call is drawn where it happened in the transcript rather than at the end of the lane's text, and it is an object a reader scans: a chip naming the tool and its state, with what it was about beside it and the detail behind it. `zig build screenshot` runs a turn carrying the four shapes a chip draws - a finished read, an edit with a diff, a failed command, and a running one - and then clicks the edit's chip through a real SDL mouse event. The gate requires the words the chips showed and that the click changed whether the call is open: a chip that stopped naming its state, or one no click can open, is a call the reader cannot see into. |
