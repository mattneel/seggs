...
| `.github/workflows/ci.yml` | The core job installs Zig, runs the Zig tests, the Python fixtures, the repository contracts, and builds the book. The native job bootstraps SDL, runs `verify`, builds the extension bundles, and runs the screenshot gate under Xvfb with the lavapipe driver, then the display smoke. A failing comparison reports the span the captures differ in, a per-eighth histogram, and uploads both frames. |
