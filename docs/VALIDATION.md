...
| `capture` | A frame can be written in the format its name asks for, which is what a QA run that has to be looked at afterwards depends on. The gate runs `--screenshot` with each extension and reads what landed rather than trusting the writer: `.png` reports `PNG image data, 1440 x 900, 8-bit/color RGBA`, `.bmp` a Windows 3.x bitmap at the same size, `.jpg` JFIF baseline, `.gif` version 89a, and `.ppm` a Netpbm pixmap. An extension none of them claims is refused by name - `UnknownCaptureFormat` - rather than guessed at, because a run that silently got a different format than it asked for is worse than one that failed. The pixels are read back from the GPU in the swapchain's own order, so the frame is opened and looked at once: a channel swap still produces a valid PNG, and it was the picture rather than the file type that ruled it out. |
| `formula` | Display mathematics is typeset rather than shown as source. This is not a property of the engine: asking it to lay a formula out succeeds while the transcript still draws the LaTeX, and the row exists because that is exactly what happened. Three things have to hold and only looking at the panel shows all three - the parser has to recognise the form the agent wrote, the engine has to lay it out, and the row has to claim the height it draws over, since the walk gives every row one line of height and a fraction is taller than that. A native test asserts the middle of it from markdown to rows: both the one-line and the opened-and-closed forms yield a row carrying a layout, no run in any row holds the source, and the rows the formula occupies are marked as its own. The gate runs `--exercise-formula`, which prompts the local mock for a fraction, an integral, and a matrix, and reads the frame: the fraction draws stacked with its rule, the integral with a radical and a nested fraction, and the matrix inside stretched parentheses, with no backslash anywhere in the panel. |
...
| `python3 tools/check_repo.py` | `PASS: 201 repository files, 72 Zig sources, 301 declared Zig tests` and `PASS: relative imports, JSON configs, Python syntax, local links, GPU/ACP source contracts, CI definition targets, and font exclusion`. It resolves every `zig build` target and every script the CI workflow names against the tree, so a renamed target cannot wait for a runner to fail. It checks Zig delimiters and selected GPU/ACP source contracts; it does not parse Zig types or compile native code. |
| `zig fmt --check build.zig src` | Passes as the tree stands. One spelling difference is worth recording, because it is easy to "correct" wrongly: the pinned formatter rewrites `@intFromEnum(x)` as `@backingInt(x)`, so a source that writes the former reports a diff from `--check` until `zig fmt` has run - and a reader who then edits the source back by hand reintroduces it. The two are the same builtin and the file compiles either way; the tree carries the formatter's spelling, which is why the check is clean. |
